...
class AnimateFromCoeff:
    ...
    def write_video(self, video, dst_path):
        # iteratively write video to disk
        writer = cv2.VideoWriter(
            dst_path, cv2.VideoWriter_fourcc(*"MP4V"), 30, (512, 512)
        )
        for frame in tqdm(video, desc="Writing Video"):
            # write frames on the fly, instead of loading all frames into memory
            frame = np.transpose(frame.data.cpu().numpy(), [1, 2, 0])
            writer.write(frame)
        writer.release()
