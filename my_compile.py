...
class TorchCompile(nn.Module):
    def __init__(self, model:nn.Module, file_path='test.onnx') -> None:
        super().__init__()
        self.session = None
        self.model = model
    ...
    def forward(self, input_tensor):
        if self.session is None:
            ...
            print(f"Compiled {model} to {self.file_path}!")
        # return model(input_tensor)
        outputs = self.session.run(None, {"input":input_tensor.long().numpy()})
        return outputs
